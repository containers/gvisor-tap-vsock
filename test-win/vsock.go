...
// copypaste from https://github.com/containers/podman/blob/main/pkg/machine/sockets/vsock.go
...
func findOpenHVSockPort() (uint64, error) {
	...
	for i := 0; i < 10; i++ {
		...
		if port < 0 || port > math.MaxUint16 {
			return 0, fmt.Errorf("invalid TCP port from random port probe: %d", port)
		}
		portU := uint64(port)
		// Try and load registry entries by port to see if they exist
		_, err = LoadHVSockRegistryEntry(portU)
		if err == nil {
		...
		// something went wrong (err is non-nil here: not ErrNotExist and entry did not load)
		return 0, err
	}
...
func (hv *HVSockRegistryEntry) Listener() (net.Listener, error) {
	if hv.Port > math.MaxUint32 {
		return nil, fmt.Errorf("hvsock port out of range for service ID: %d", hv.Port)
	}
	svc := winio.VsockServiceID(uint32(hv.Port))
	n := winio.HvsockAddr{
...
func (hv *HVSockRegistryEntry) ListenSetupWait() (func() error, io.Closer, error) {
	...
	go ListenAndWaitOnSocket(errChan, listener)
	return func() error {
		return <-errChan
	}, listener, nil
}
...
func loadHVSockRegistryEntries(purpose HVSockPurpose, limit int) ([]*HVSockRegistryEntry, error) {
	...
	for _, subKeyName := range subKeyNames {
		...
		if toolName != GvisorTestToolName {
			continue
		}
