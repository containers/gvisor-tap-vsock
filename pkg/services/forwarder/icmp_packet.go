...
// createICMPConnection creates an ICMP connection using platform-appropriate socket type.
// On Windows, it uses privileged raw sockets (ip4:icmp).
// On Linux/macOS, it uses unprivileged ICMP sockets (udp4).
func createICMPConnection() (*netIcmp.PacketConn, error) {
	...
	if runtime.GOOS == "windows" {
		...
		// Linux/macOS can use unprivileged ICMP sockets (udp4)
		network = "udp4"
	}

	conn, err := netIcmp.ListenPacket(network, "0.0.0.0")
	if err != nil {
...
func sendEchoRequest(conn *netIcmp.PacketConn, destAddr tcpip.Address, ident, seq uint16, payload []byte) error {
	...
	if runtime.GOOS == "windows" {
		...
		dst = &net.UDPAddr{IP: dstIP, Port: 0}
	}
