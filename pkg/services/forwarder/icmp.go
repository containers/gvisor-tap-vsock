...
func ICMP(s *stack.Stack, nat map[tcpip.Address]tcpip.Address, natLock *sync.Mutex) *Forwarder {
	return NewForwarder(s, func(r *ICMPForwarderRequest) {
		...
		pkt := r.Packet()
		if pkt == nil {
			log.Debugf("Dropping ICMP packet from VM (no packet data)")
			return
		}
		...
		if len(transportHeader) < header.ICMPv4MinimumSize {
			log.Debugf("Dropping ICMP packet from VM (packet too short)")
			return
		}
		...
		if icmpHeader.Type() != header.ICMPv4Echo {
			// Not a PING, drop it
			log.Debugf("Dropping ICMP packet from VM (type %d, not Echo Request)", icmpHeader.Type())
			return
		}
